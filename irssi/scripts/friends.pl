...
# release note, if any
my $release_note = "Please read http://irssi.atn.pl/friends/current/README\n";
			...
			if ($revengemode > 0) {
				...
				if ($kickerIdx < 0) {
					push(@friends, new_friend(
						...
						undef,				# globflags
						$noPrefix.",D,0",		# channel,chanflags,chandelay
						undef,				# password
				...
				if ($revengemode > 1 && $channelInfo->{chanop}) {
					# 2 Deop the user, give them the +D flag for the channel, and kick them.
					$channelInfo->command("KICK ". $channel . " ".$kicker. " Don't mess with my friends");
					if ($revengemode > 2) {
						# 3 Deop the user, give them the +D flag for the channel, kick, and ban them.
